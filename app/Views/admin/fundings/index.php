...
<div style="display: flex; gap: 8px; margin-bottom: 20px; flex-wrap: wrap;">
    <a href="<?= base_url('admin/fundings') ?>" class="btn <?= !$filter ? 'btn-primary' : 'btn-outline' ?>">
        All (<?= array_sum($counts) ?>)
    </a>
    <a href="<?= base_url('admin/fundings?status=pending') ?>" class="btn <?= $filter === 'pending' ? 'btn-primary' : 'btn-outline' ?>">
        Pending (<?= $counts['pending'] ?>)
    </a>
    <a href="<?= base_url('admin/fundings?status=processing') ?>" class="btn <?= $filter === 'processing' ? 'btn-primary' : 'btn-outline' ?>">
        Processing (<?= $counts['processing'] ?>)
    </a>
    <a href="<?= base_url('admin/fundings?status=approved') ?>" class="btn <?= $filter === 'approved' ? 'btn-primary' : 'btn-outline' ?>">
        Approved (<?= $counts['approved'] ?>)
    </a>
    <a href="<?= base_url('admin/fundings?status=rejected') ?>" class="btn <?= $filter === 'rejected' ? 'btn-primary' : 'btn-outline' ?>">
...
<div class="card">
    <div class="card-header">
        <h3 class="card-title">Company Funding Requests</h3>
    </div>
    <div class="card-body">
        <?php if (empty($fundings)): ?>
            <p style="color: var(--gray-500); text-align: center; padding: 40px;">No funding requests found</p>
        <?php else: ?>
                    ...
                    <tbody>
                        <?php foreach ($fundings as $funding): ?>
                            <tr>
                                ...
                                <td>
                                    <?= esc($funding['company_name']) ?>
                                    <div style="font-size: 0.75rem; color: var(--gray-500);"><?= esc($funding['company_email']) ?></div>
                                </td>
                                ...
                                <td>
                                    <?php if (!empty($funding['receipt_path'])): ?>
                                        <a href="<?= base_url('admin/fundings/' . $funding['id'] . '/receipt') ?>" target="_blank" class="btn btn-outline" style="padding: 2px 8px; font-size: 0.7rem;">
                                            View Receipt
                                        </a>
                                    <?php else: ?>
                                ...
                                <td>
                                    <a href="<?= base_url('admin/fundings/' . $funding['id']) ?>" class="btn btn-outline" style="padding: 4px 12px; font-size: 0.75rem;">View</a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
